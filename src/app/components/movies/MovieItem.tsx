    ...
    <Card className="group overflow-hidden transition-all duration-500 ease-out hover:scale-[1.03] hover:rotate-[0.3deg] hover:shadow-2xl dark:hover:shadow-indigo-900/40">
      <Link href={`/movie/${result.id}`} className="block">
        <div className="relative w-full h-72 rounded-t-xl overflow-hidden">
          ...
          {/* Navigate (changed to button, no nested Link) */}
          <button
            type="button"
            onClick={() => (window.location.href = `/movie/${result.id}`)}
            className="absolute bottom-3 right-3 bg-indigo-600 hover:bg-indigo-700 text-white p-2 rounded-full shadow-lg transition-transform duration-300 group-hover:scale-110 group-hover:rotate-6"
          >
